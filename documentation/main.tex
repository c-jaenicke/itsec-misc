...
\noindent Die nächste Abbildung zeigt das Ergebnis des Pings mit allen Ports geschlossen und den Production-Regeln:
\begin{figure}[H]
	\centering
...
\end{figure}
\noindent Man kann sehen, dass keiner der Pings erfolgreich war. Das liegt daran, dass bei beiden Regelsätzen das ICMP-Protokoll, welches für Pings genutzt wird, blockiert wird.
...
\end{figure}
\noindent Hervorzuheben ist, dass der Scan des Webservers nur den Port 22 (SSH) gefunden hat, nicht aber den Port 80 für den NGINX-Server. Die Regel, dass Port 80 auf dem Webserver nur Anfragen von der IP-Adresse der Firewall annimmt, funktioniert dementsprechend.
Zudem war Suricata in der Lage den Nmap-Scan, den Script-Scan, zu entdecken und zu melden.
\begin{figure}[!ht]
	\centering
	\includegraphics[width=\textwidth]{suricata-detection-nmap-scan.png}
...
\noindent Die folgende Abbildung zeigt die Auslastung der Firewall, wie sie in htop dargestellt wird:
\begin{figure}[H]
	\centering
	...
	\label{fig:dos-htop}
\end{figure}
\noindent htop ist ein Tool, welches die derzeitige Auslastung der Hardware eines Systems grafisch darstellt. Man kann sehen, dass die CPU zu fast 100\% ausgelastet ist. Die Firewall ist nicht mehr in der Lage, auf andere Anfragen zu antworten, und andere Prozesse werden deutlich verlangsamt. \\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{suricata-spam-dos.png}
...
Für detaillierte Informationen zur Konfiguration der Firewall und des Webservers siehe Kapitel \ref{firewall-info} und \ref{webserver-info}.
\\ \\
Im Folgenden wird die Aufsetzung einer Webanwendung mit zugehöriger Datenbank sowie die Durchführung von SQL Injection und Cross-Site Scripting Angriffen beschrieben.

\subsection{Konfiguration des Webservers und der Datenbank}
...
\subsection{Durchführung einer SQL Injection Attacke}
...
\subsection{Durchführung von Cross-Site Scripting}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{xss-keylogger-network-traffic-2.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{xss-phishing-page-injection.png}
...
\subsection{Netzwerküberwachung einrichten}
\subsubsection{Konfiguration des Intrusion Detection Systems}\label{config-ids}
...
Im Folgenden wird eine zweistufige forensische Analyse durchgeführt: Zunächst wird ein Speicherabbild der zweiEnergie AG mittels Volatility untersucht, gefolgt von einer detaillierten Analyse von Netzwerkmitschnitten aus einem Atomkraftwerk des Unternehmens.

\subsection{Aufgabe 1}
Darstellung forensisches Vorgehen, Datensammlung, Datenanalyse, Grenzen der Analyse
\\ \\
\textbf{a) Welche Methodik der Forensik würden Sie zum Sichern des RAMS nutzen?}
...
Dazu stehen verschiedene Software-Tools bereit, wie DumpIt \cite{dumpit} für Windows-Systeme und LiME \cite{lime} für Linux-basierte Systeme.
\\ \\
\textbf{b) Worauf mussten Sie bei dieser Sicherungsmethodik besonders achten?}
Das System darf nach der Infizierung und vor der Sicherung der Daten, nicht heruntergefahren oder vom Strom getrennt werden. Die flüchtige Daten im RAM können sonst verloren gehen. \\ \\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{vol-prozesse-verstekt.png}
	\caption{Volatility: Versteckte Prozesse}
	\label{fig:vol-prozesse-verstekt.png}
\end{figure} 
\noindent \textbf{Welche Netzwerkverbindungen bestehen bzw. bestanden? Ist etwas auffällig?}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{vol-verbindungen.png}
	\caption{Volatility: bestehende Verbindungen}
	\label{fig:vol-verbindungen.png}
\end{figure} 
\noindent \textbf{Welche Prozesse haben welche Verbindung gestartet?}
...
\begin{table}[H]
	...
	\end{center}
\end{table}
\noindent \textbf{Untersuchen Sie die Eingaben in der command line. Welche Kommandos wurden ausgeführt? Fällt Ihnen etwas auf?}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{vol-cmd-history.png}
	\caption{Volatility: Eingaben in der Kommandozeile}
	\label{fig:vol-cmd-history}
\end{figure} 
\noindent \textbf{Welche verdächtigen Services sind derzeit aktiv?}
...
\\ \\
\textbf{a) Bestimmen Sie alle Netzwerkteilnehmer (MAC, IPv4), die in den Netzwerkausschnitten auftauchen. Broadcast-Adressen können Sie ausschließen. Tipp: Betrachten Sie auch das Protokoll LLDP.}
\\
\begin{table}[H]
    \centering
...
\footnotetext[1]{Diese Multicast-Adressen sind nicht an eine bestimmte Netzwerkkarte gebunden, sondern können von mehreren Geräten im Netzwerk empfangen werden, die der entsprechenden Multicast-Gruppe beigetreten sind. \cite{multicast-ip}}
\footnotetext[2]{Bei der Paketanalyse wurde die IP-Adresse 23.95.230.107 als Ziel-IP für Pakete beobachtet, die über dieses 3Com-Gerät geroutet wurden. Diese IP-Adresse gehört nicht zum 3Com-Gerät selbst, sondern zeigt an, dass das Gerät als Router oder Gateway für den Verkehr zu dieser externen Adresse fungiert.}
...
\textbf{b) Bestimmen Sie, welche Protokolle verwendet werden.}
\\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth, height=15cm]{diagramm-teilnehmer.png}
...
\\ \\
\textbf{c) Bei welchem System handelt es sich um das infizierte System und wieso ist dieses System verdächtig? Beachten Sie die Baseline (normal.pcapng), die Protokolle TCP und UDP sowie die verwendeten Ports.}
\\ 
...
\end{figure}
\textbf{d) Wann hat der Rechner sich versucht mit einem Host außerhalb des Netzwerks zu verbinden?}
\\
Am 8. Juni 2018 um 13:46:47.788372000 CEST wurde ein UDP-Paket (Frame 96) vom System zur Adresse 234.5.6.7 auf Port 8910 gesendet. Kurz darauf folgten weitere Pakete (Frames 97, 103, 466, 467, 473, 644, 645). Diese IP-Adresse gehört zum reservierten Bereich für Multicast-Adressen (224.0.0.0 bis 239.255.255.255) \cite{multicast-ip}. Obwohl es sich nicht um eine Kommunikation mit einem externen Host handelt, ist dieses Verhalten dennoch auffällig. In der normalen Baseline-Aktivität wurde Multicast-Verkehr hauptsächlich im Zusammenhang mit LLMNR (Link-Local Multicast Name Resolution) oder SSDP (Simple Service Discovery Protocol) beobachtet.
\\ \\
Am 8. Juni 2018 um 13:46:52.993988000 CEST initiiert das System einen TCP-Verbindungsversuch 
(Frame 207) zu einem tatsächlich externen Host (23.95.230.107) auf Port 80. Dies ist der erste beobachtete Versuch, eine Verbindung außerhalb des lokalen Netzwerks herzustellen. Diesem folgten kurz darauf weitere Pakete (Frames 296, 363, 493, 537).

\subsubsection{Phase 2}
Lateral Movement – Offline Modus (attack2.pcapng)
\\ \\
\textbf{e) Welche Auffälligkeiten sehen Sie in diesem Mitschnitt? Welches Protokoll wird häufig verwendet?}
\\
...
\textbf{f) Welches System scannt das Netzwerk nach Netzwerkteilnehmern?}
\\
...
\\ \\
\textbf{g) Über welchen Port versucht das infizierte System Kontakt aufzunehmen?}
\\
...
\\ \\
\textbf{h) Schauen Sie sich die einzelnen Verbindungsversuche näher an. Welches Protokoll wird neu verwendet?}
\\
...
\\ \\
\textbf{i) Wurde die Verbindung erfolgreich hergestellt? Notieren Sie ggf. den Zeitstempel.}
\\
...
\\ \\ 
\textbf{j) Schauen Sie sich die einzelnen TCP-Sessions an. (Filter: tcp.stream == <n>) Sie suchen die Session, bei welcher die Teilnehmer 10.3.5.3 und 10.3.5.5 miteinander kommunizieren.}
\\
In der relevanten TCP-Session (tcp.stream == 1) zwischen 10.3.5.3 und 10.3.5.5 ist eine VNC-Sitzung zu beobachten, in der verschiedene Client-Pointer-Events ausgeführt werden. Dies deutet auf eine aktive Fernsteuerung hin.
...
\textbf{k) Gegeben sind folgende Richtlinien:
\begin{itemize}
    \item Der SCADA-Arbeitsplatz ist ausschließlich für die Steuerung der PLCs verantwortlich.
    \item Der Techniker-Arbeitsplatz wartet den Programmcode der PLCs und die entsprechende Anwendung auf dem SCADA-Arbeitsplatz mittels normalem VNC (TCP-Port 5900).
\end{itemize}
...
\subsection{Firewall}\label{firewall-info}
...
\subsection{Webserver}\label{webserver-info}
...
\begin{enumerate}
	\item Das jeweils relevante Firewall-Skript wurde ausführbar gemacht: \\ \lstinline[breaklines]|$ sudo chmod +x <name des skripts>|.
